...
import { base64ToBytes, hexToBytes, textToBytes } from '../../src/encryption/encodeDecode';
...
describe('RSA.ts', () => {
  ...
  beforeEach(() => {
    vi.stubGlobal('crypto', {
      subtle: {
        digest: async (type: string, arr: Uint8Array) => {
          return await Promise.resolve(hexToBytes(createHash('sha1').update(arr).digest('hex')));
        }
      },
  ...
  it('can decrypt some data, encrypted with a tool on the web, with a private key', async () => {
    ...
    const output = await decrypt(input, privateKey);

    expect(output).toEqual(expectedOutput);
  });

  it('can decrypt some data, encrypted with a tool on the web, with a public key', async () => {
    const input = base64ToBytes(
    ...
    const output = await decrypt(input, publicKey);

    expect(output).toEqual(expectedOutput);
  });

  it('can encrypt some data with a public key and decrypt it back to the same data with the private key', async () => {
    const input = textToBytes('This is a test string');

    const encrypted = await encrypt(input, publicKey);
  ...
  it('can encrypt some data with a private key and decrypt it back to the same data with the public key', async () => {
    const input = textToBytes('This is a test string');

    const encrypted = await encrypt(input, privateKey);
  ...
  it('can encrypt (private) and decrypt (public) a string which is longer than what can fit in one block', async () => {
  ...
  it('can encrypt (public) and decrypt (private) a string which is longer than what can fit in one block', async () => {
    const input = textToBytes(
      "This is a test string and it has been made very long to test whether the encrypt() and decrypt() functions can handle messages that can't fit into one block 1234123412341234123412341234123412341234123412341234123412341234"
    );
