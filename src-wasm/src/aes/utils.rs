// Public functions
pub fn key_expansion(key: &[u8], rounds: &usize, words_in_key: &usize) -> Vec<Vec<u8>> {
  let mut expanded_key: Vec<Vec<u8>> = Vec::new();
  let mut i: usize = 0;

  while i < *words_in_key {
    expanded_key.push(vec![0; 4]);
    expanded_key[i].copy_from_slice(&key[ 4 * i .. 4 * (i + 1) ]);
    i += 1;
  }

  while i < 4 * (*rounds + 1) {
    let mut temp = expanded_key[i - 1].clone();
    if i % words_in_key == 0 {
      temp = xor_words(&sub_bytes(&rot_word(&temp)), &R_CON[i / words_in_key].to_vec());
    } else if *words_in_key > 6 && i % words_in_key == 4 {
      temp = sub_bytes(&temp);
    }
    expanded_key.push(xor_words(&expanded_key[i - words_in_key].clone(), &temp));
    i += 1;
  }
  
  expanded_key
}

pub fn add_round_key(input: &[u8], key_words: &[Vec<u8>]) -> Vec<u8> {
  let mut output: Vec<u8> = Vec::new();
  for i in 0..4 {
    output.append(&mut xor_words(&input[4 * i .. 4 * (i + 1)], &key_words[i]));
  }
  output
}

pub fn sub_bytes(word: &[u8]) -> Vec<u8> {
  let mut result = Vec::new();
  for value in word {
    result.push(S_BOX[*value as usize]);
  }
  result
}

pub fn inv_sub_bytes(word: &[u8]) -> Vec<u8> {
  let mut result = Vec::new();
  for value in word {
    result.push(INV_S_BOX[*value as usize]);
  }
  result

}

pub fn shift_rows(input: &[u8]) -> Vec<u8> {
  vec![
    input[0],
    input[5],
    input[10],
    input[15],
    input[4],
    input[9],
    input[14],
    input[3],
    input[8],
    input[13],
    input[2],
    input[7],
    input[12],
    input[1],
    input[6],
    input[11],
  ]
}

pub fn inv_shift_rows(input: &[u8]) -> Vec<u8> {
  vec![
    input[0],
    input[13],
    input[10],
    input[7],
    input[4],
    input[1],
    input[14],
    input[11],
    input[8],
    input[5],
    input[2],
    input[15],
    input[12],
    input[9],
    input[6],
    input[3],
  ]
}

pub fn mix_columns(s: &[u8]) -> Vec<u8> {
  let mut out = vec![0; 16];
  out[0] = gmul2(s[0]) ^ gmul3(s[1]) ^ s[2] ^ s[3];
  out[1] = s[0] ^ gmul2(s[1]) ^ gmul3(s[2]) ^ s[3];
  out[2] = s[0] ^ s[1] ^ gmul2(s[2]) ^ gmul3(s[3]);
  out[3] = gmul3(s[0]) ^ s[1] ^ s[2] ^ gmul2(s[3]);

  out[4] = gmul2(s[4]) ^ gmul3(s[5]) ^ s[6] ^ s[7];
  out[5] = s[4] ^ gmul2(s[5]) ^ gmul3(s[6]) ^ s[7];
  out[6] = s[4] ^ s[5] ^ gmul2(s[6]) ^ gmul3(s[7]);
  out[7] = gmul3(s[4]) ^ s[5] ^ s[6] ^ gmul2(s[7]);

  out[8] = gmul2(s[8]) ^ gmul3(s[9]) ^ s[10] ^ s[11];
  out[9] = s[8] ^ gmul2(s[9]) ^ gmul3(s[10]) ^ s[11];
  out[10] = s[8] ^ s[9] ^ gmul2(s[10]) ^ gmul3(s[11]);
  out[11] = gmul3(s[8]) ^ s[9] ^ s[10] ^ gmul2(s[11]);

  out[12] = gmul2(s[12]) ^ gmul3(s[13]) ^ s[14] ^ s[15];
  out[13] = s[12] ^ gmul2(s[13]) ^ gmul3(s[14]) ^ s[15];
  out[14] = s[12] ^ s[13] ^ gmul2(s[14]) ^ gmul3(s[15]);
  out[15] = gmul3(s[12]) ^ s[13] ^ s[14] ^ gmul2(s[15]);

  out
}

pub fn inv_mix_columns(s: &[u8]) -> Vec<u8> {
  let mut out = vec![0; 16];
  out[0] = gmule(s[0]) ^ gmulb(s[1]) ^ gmuld(s[2]) ^ gmul9(s[3]);
  out[1] = gmul9(s[0]) ^ gmule(s[1]) ^ gmulb(s[2]) ^ gmuld(s[3]);
  out[2] = gmuld(s[0]) ^ gmul9(s[1]) ^ gmule(s[2]) ^ gmulb(s[3]);
  out[3] = gmulb(s[0]) ^ gmuld(s[1]) ^ gmul9(s[2]) ^ gmule(s[3]);

  out[4] = gmule(s[4]) ^ gmulb(s[5]) ^ gmuld(s[6]) ^ gmul9(s[7]);
  out[5] = gmul9(s[4]) ^ gmule(s[5]) ^ gmulb(s[6]) ^ gmuld(s[7]);
  out[6] = gmuld(s[4]) ^ gmul9(s[5]) ^ gmule(s[6]) ^ gmulb(s[7]);
  out[7] = gmulb(s[4]) ^ gmuld(s[5]) ^ gmul9(s[6]) ^ gmule(s[7]);

  out[8] = gmule(s[8]) ^ gmulb(s[9]) ^ gmuld(s[10]) ^ gmul9(s[11]);
  out[9] = gmul9(s[8]) ^ gmule(s[9]) ^ gmulb(s[10]) ^ gmuld(s[11]);
  out[10] = gmuld(s[8]) ^ gmul9(s[9]) ^ gmule(s[10]) ^ gmulb(s[11]);
  out[11] = gmulb(s[8]) ^ gmuld(s[9]) ^ gmul9(s[10]) ^ gmule(s[11]);

  out[12] = gmule(s[12]) ^ gmulb(s[13]) ^ gmuld(s[14]) ^ gmul9(s[15]);
  out[13] = gmul9(s[12]) ^ gmule(s[13]) ^ gmulb(s[14]) ^ gmuld(s[15]);
  out[14] = gmuld(s[12]) ^ gmul9(s[13]) ^ gmule(s[14]) ^ gmulb(s[15]);
  out[15] = gmulb(s[12]) ^ gmuld(s[13]) ^ gmul9(s[14]) ^ gmule(s[15]);

  out
}

// Helper variables
const S_BOX: [u8; 256] = [
  ,
];

const INV_S_BOX: [u8; 256] = [
  ,
];


const R_CON: [[u8; 4]; 11] = [
  [0x00, 0x00, 0x00, 0x00],
  [0x01, 0x00, 0x00, 0x00],
  [0x02, 0x00, 0x00, 0x00],
  [0x04, 0x00, 0x00, 0x00],
  [0x08, 0x00, 0x00, 0x00],
  [0x10, 0x00, 0x00, 0x00],
  [0x20, 0x00, 0x00, 0x00],
  [0x40, 0x00, 0x00, 0x00],
  [0x80, 0x00, 0x00, 0x00],
  [0x1b, 0x00, 0x00, 0x00],
  [0x36, 0x00, 0x00, 0x00]
];

// Helper functions

fn xor_words(x: &[u8], y: &[u8]) -> Vec<u8> {
  let mut result: Vec<u8> = Vec::new();
  for i in 0..4 {
    result.push(x[i] ^ y[i]);
  }

  result
}

fn rot_word(word: &[u8]) -> Vec<u8> {
  vec![word[1], word[2], word[3], word[0]]
}

fn gmul2(a: u8) -> u8 {
  let h = a & 0x80;
  let mut b = a << 1;
  if h == 0x80 {
    b = b ^0x1b;
  }
  b
}

fn gmul3(a: u8) -> u8 {
  a ^gmul2(a)
}

fn gmul9(a: u8) -> u8 {
  a ^gmul2(gmul2(gmul2(a)))
}

fn gmulb(a: u8) -> u8 {
  a ^ gmul2(a) ^ gmul2(gmul2(gmul2(a)))
}

fn gmuld(a: u8) -> u8 {
  a ^ gmul2(gmul2(a)) ^ gmul2(gmul2(gmul2(a)))
}

fn gmule(a: u8) -> u8 {
  gmul2(a) ^ gmul2(gmul2(a)) ^ gmul2(gmul2(gmul2(a)))
}